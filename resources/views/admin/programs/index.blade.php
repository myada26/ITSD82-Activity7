    ...
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        ...
        <div class="flex flex-wrap gap-2.5">
            <button @click="open = 'bulk'" class="px-4 py-2 rounded-lg text-[13.5px] font-bold flex items-center gap-2 bg-white border-2 border-green-200 hover:border-green-600 hover:text-green-600 text-green-400 transition-all shadow-sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>Import List
            </button>
    ...
    <div class="bg-white rounded-xl border border-green-200 shadow-sm overflow-hidden">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-6 py-5 border-b border-[#eaf0ec]">
            ...
            <form method="GET" action="{{ route('admin.programs.index') }}" class="flex flex-wrap items-center gap-2.5">
                ...
                <select name="department_id" onchange="this.form.submit()" class="border-2 border-green-200 rounded-lg py-2 px-3 text-[13px] font-medium text-green-400 outline-none focus:border-green-600 bg-white cursor-pointer transition-colors">
                    <option value="">All Departments</option>
                    @foreach($departments as $d)
                        <option value="{{ $d->id }}" {{ request('department_id') == $d->id ? 'selected' : '' }}>{{ $d->code }}</option>
                    @endforeach
                </select>
            </form>
        </div>
            ...
            <table class="w-full text-left border-collapse min-w-[700px]">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-[#eaf0ec] hover:bg-[#f0f3f1]/50 transition-colors last:border-b-0 group">
                        ...
                        <td class="px-6 py-4 text-[13.5px] font-semibold text-green-400">{{ $prog->department->code ?? '—' }}</td>
