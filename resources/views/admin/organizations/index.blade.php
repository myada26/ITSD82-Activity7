...
<div class="max-w-6xl mx-auto pb-10" x-data="{ open: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
            <h2 class="text-[22px] font-bold text-green-800">Manage Organizations</h2>
            <p class="text-[13.5px] text-green-400 mt-1 font-medium">Each organization is scoped to a hierarchy level (SSC / College / Department)</p>
        </div>
        <div class="flex flex-wrap gap-2.5">
            <a href="{{ route('admin.organizations.index', ['export' => 'csv']) }}" class="px-4 py-2 rounded-lg text-[13.5px] font-bold flex items-center gap-2 bg-white border-2 border-green-200 hover:border-green-600 hover:text-green-600 text-green-400 transition-all shadow-sm">
    ...
    <div class="bg-white rounded-xl border border-green-200 shadow-sm overflow-hidden">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-6 py-5 border-b border-[#eaf0ec]">
            <div>
                <h3 class="text-[15px] font-bold text-green-800">All Organizations</h3>
                <p class="text-[12.5px] text-green-300 font-medium mt-0.5">{{ $organizations->total() }} total records</p>
            </div>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-green-50 border-b border-green-200">
                        ...
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">Type</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">Parent Unit</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">Users</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-[#eaf0ec] hover:bg-[#f0f3f1]/50 transition-colors last:border-b-0 group">
                        <td class="px-6 py-4 text-[14px] font-bold text-green-800">{{ $org->name }}</td>
                        <td class="px-6 py-4"><span class="font-mono text-[12px] font-bold text-green-600 bg-green-100 px-2 py-1 rounded-md">{{ $org->type }}</span></td>
                        <td class="px-6 py-4 text-[13.5px] font-semibold text-green-400">{{ $org->college->code ?? $org->department->code ?? '—' }}</td>
                        <td class="px-6 py-4 text-[13.5px] font-semibold text-green-400">{{ $org->users_count ?? $org->users()->count() }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.organizations.edit', $org) }}" class="p-1.5 rounded-lg text-green-300 hover:bg-green-200 hover:text-green-600 transition-colors">
                    ...
                    @empty
                    <tr><td colspan="6" class="px-6 py-14 text-center text-[14px] font-semibold text-green-400">No organizations found</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="px-6 py-4 border-t border-[#eaf0ec] flex justify-between items-center bg-green-50">
            <span class="text-[12.5px] font-medium text-green-300">Showing {{ $organizations->firstItem() ?? 0 }}–{{ $organizations->lastItem() ?? 0 }} of {{ $organizations->total() }}</span>
    ...
    <div x-show="open" x-cloak class="fixed inset-0 z-40 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="relative bg-white rounded-xl w-full max-w-lg shadow-2xl z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#eaf0ec] shrink-0">
                <div><h2 class="text-[18px] font-bold text-green-800">Add New Organization</h2><p class="text-[13px] text-green-400 mt-0.5 font-medium">Create an organization scoped to a hierarchy level.</p></div>
            ...
            <form method="POST" action="{{ route('admin.organizations.store') }}" class="flex flex-col min-h-0">
                ...
                <div class="p-6 overflow-y-auto space-y-5">
                    <div>
                        <label class="block text-[13px] font-semibold text-green-400 mb-2">Organization Name <span class="text-red-500">*</span></label>
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. COE Student Council" class="w-full px-4 py-2.5 border-2 border-green-200 rounded-lg bg-white text-[14px] font-medium text-green-800 outline-none focus:border-green-600 transition-colors">
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-[13px] font-semibold text-green-400 mb-2">Parent College (if applicable)</label>
                    ...
                    <div>
                        <label class="block text-[13px] font-semibold text-green-400 mb-2">Parent Department (if applicable)</label>
                ...
                <div class="px-6 py-4 border-t border-[#eaf0ec] bg-green-50 flex justify-end gap-3 shrink-0 rounded-b-2xl">
                    ...
                    <button type="submit" class="px-4 py-2 rounded-lg text-[13.5px] font-bold bg-green-600 hover:bg-green-500 text-white border-2 border-transparent transition-all shadow-sm">Save Organization</button>
                </div>
            </form>
        </div>
    </div>
