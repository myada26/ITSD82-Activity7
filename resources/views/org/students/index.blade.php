...
<div class="max-w-6xl mx-auto pb-10" x-data="{ open: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-[13.5px] text-green-400 mt-1 font-medium">Scoped to {{ auth()->user()->organization?->name ?? 'your organization' }} · Active Semester</p>
        </div>
            ...
            <table class="w-full text-left border-collapse min-w-[700px]">
                <thead>
                    <tr class="bg-green-50 border-b border-green-200">
                        ...
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">Membership</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-[#eaf0ec] hover:bg-[#f0f3f1]/50 transition-colors last:border-b-0 group">
                        ...
                        <td class="px-6 py-4 text-[13.5px] font-semibold text-green-400">{{ $student->membership_type ?? 'Regular' }}</td>
                        <td class="px-6 py-4">
    ...
    <div x-show="open === 'manual'" x-cloak class="fixed inset-0 z-40 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="relative bg-white rounded-xl w-full max-w-2xl shadow-2xl z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#eaf0ec] shrink-0">
                <div>
                    <h2 class="text-[18px] font-bold text-green-800">Enroll New Student</h2>
                    <p class="text-[13px] text-green-400 mt-0.5 font-medium">Add a student to your organization's roster.</p>
                </div>
        ...
        <div class="relative bg-white rounded-xl w-full max-w-lg shadow-2xl z-10">
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#eaf0ec]">
                <div>
                    ...
                    <p class="text-[13px] text-green-400 mt-0.5">Upload a CSV or Excel file to import multiple students.</p>
                </div>
                <button @click="open = false" class="text-green-300 hover:bg-[#f0f3f1] p-2 rounded-lg transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg></button>
            ...
            <form method="POST" action="{{ route('org.students.import.store') }}" enctype="multipart/form-data" class="p-6">
                @csrf
                <div class="flex flex-col items-center border-2 border-dashed border-green-200 rounded-lg bg-green-50 p-8 text-center mb-5">
                    <svg class="w-12 h-12 text-green-600 mb-4 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                    </svg>
                    <p class="text-[13px] text-green-400 mb-4">Supported: .csv, .xlsx, .xls (max 5MB)</p>
                    <input type="file" name="file" accept=".csv,.xlsx,.xls" class="text-[13px] text-green-400">
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" @click="open = false" class="px-4 py-2 rounded-lg text-[13.5px] font-bold bg-white border-2 border-green-200 hover:border-green-600 hover:text-green-600 text-green-400 transition-all">Cancel</button>
                    <button type="submit" class="px-4 py-2 rounded-lg text-[13.5px] font-bold bg-green-600 hover:bg-green-500 text-white transition-all shadow-sm">Process Upload</button>
                </div>
            </form>
        </div>
    </div>
