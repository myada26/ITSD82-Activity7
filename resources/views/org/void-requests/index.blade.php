...
<div class="max-w-6xl mx-auto pb-10" x-data="{ open: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
            <h2 class="text-[22px] font-bold text-green-800">Void Requests</h2>
            <p class="text-[13.5px] text-green-400 mt-1 font-medium">Request to void erroneous transactions pending supervisor review</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-green-200 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-[#eaf0ec] flex items-center justify-between">
            <div>
                <h3 class="text-[15px] font-bold text-green-800">Void Request History</h3>
                <p class="text-[12.5px] text-green-300 font-medium mt-0.5">{{ $voidRequests->total() }} total requests</p>
            </div>
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-green-50 border-b border-green-200">
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">OR No.</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">Student</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">Amount</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">Reason</th>
                        <th class="px-6 py-3.5 text-[11.5px] font-bold text-green-300 uppercase tracking-widest">Date Requested</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-[#eaf0ec] hover:bg-[#f0f3f1]/50 transition-colors last:border-b-0">
                        <td class="px-6 py-4"><span class="font-mono text-[13px] font-bold text-green-600 bg-green-100 px-2 py-1 rounded-md">{{ $vr->transaction?->or_number }}</span></td>
                        <td class="px-6 py-4 text-[14px] font-bold text-green-800">{{ $vr->transaction?->student?->full_name ?? '—' }}</td>
                        <td class="px-6 py-4 font-mono text-[14px] font-bold text-green-800">₱{{ number_format($vr->transaction?->amount, 2) }}</td>
                        <td class="px-6 py-4 text-[13px] text-green-400 font-medium max-w-[200px] truncate">{{ $vr->reason }}</td>
                        <td class="px-6 py-4 text-[12.5px] text-green-300">{{ $vr->created_at->format('M d, Y H:i') }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            @else
                            <span class="text-[12px] text-green-300">Resolved</span>
                            @endif
                        </td>
                        @endif
                    </tr>
                    @empty
                    <tr>
                        <td colspan="{{ auth()->user()->canApproveVoid() ? 7 : 6 }}" class="px-6 py-14 text-center text-[14px] font-semibold text-green-400">No void requests submitted</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-6 py-4 border-t border-[#eaf0ec] flex justify-between items-center bg-green-50">
            <span class="text-[12.5px] font-medium text-green-300">Showing {{ $voidRequests->firstItem() ?? 0 }}–{{ $voidRequests->lastItem() ?? 0 }} of {{ $voidRequests->total() }}</span>
    ...
    <div x-show="open" x-cloak class="fixed inset-0 z-40 flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="relative bg-white rounded-xl w-full max-w-lg shadow-2xl z-10 flex flex-col max-h-[90vh]">
            <div class="flex items-center justify-between px-6 py-4 border-b border-[#eaf0ec] shrink-0">
                <div>
                    <h2 class="text-[18px] font-bold text-green-800">New Void Request</h2>
                    <p class="text-[13px] text-green-400 mt-0.5 font-medium">Submit a void request for supervisor review.</p>
                </div>
            ...
            <form method="POST" action="{{ route('org.void-requests.store') }}" enctype="multipart/form-data" class="flex flex-col min-h-0">
                ...
                <div class="p-6 overflow-y-auto space-y-5">
                    <div>
                        <label class="block text-[13px] font-semibold text-green-400 mb-2">Official Receipt Number <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-[13px] font-semibold text-green-400 mb-2">Reason for Void <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-[13px] font-semibold text-green-400 mb-2">Supporting Document <span class="text-[11px] font-normal text-green-300 ml-1">(Optional)</span></label>
                        <div class="flex flex-col items-center border-2 border-dashed border-green-200 rounded-lg bg-green-50 p-6 text-center">
                            <svg class="w-10 h-10 text-green-600 opacity-30 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                            </svg>
                            <p class="text-[12.5px] text-green-400 mb-3 font-medium">Attach proof or memo (PDF, JPG, PNG)</p>
                            <input type="file" name="document" accept=".pdf,.jpg,.jpeg,.png" class="text-[13px] text-green-400">
                        </div>
                    </div>
                </div>
                <div class="px-6 py-4 border-t border-[#eaf0ec] bg-green-50 flex justify-end gap-3 shrink-0 rounded-b-2xl">
                    ...
                    <button type="submit" class="px-4 py-2 rounded-lg text-[13.5px] font-bold bg-green-600 hover:bg-green-500 text-white border-2 border-transparent transition-all shadow-sm">Submit Request</button>
                </div>
            </form>
        </div>
    </div>
